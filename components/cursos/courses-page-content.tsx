      ...
      {error ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="animate-pulse rounded-[28px] border border-border/50 bg-card p-6 space-y-3">
        ...
        <EmptyState
          ...
          description="Crie um curso para começar a organizar suas turmas."
        />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {courses.map((course) => (
