...
function formatCPF(index) {
  return String(10000000000 + index).slice(0, 11);
}
...
async function main() {
  ...
  for (let i = 0; i < alunosData.length; i++) {
    ...
    const aluno = await prisma.aluno.create({
      data: {
        ...
        responsavelNome: `Responsável por ${nome}`,
        responsavelTelefone: formatPhone(i + 101),
        responsavelEmail: `responsavel.${emailSlug}@demo.com`,
  ...
  for (const evento of eventosData) {
    ...
    const fim = new Date(evento.dataInicio);
